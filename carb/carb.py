...
class Benchmark:
    ...
    def compare(self, predicted, matchingFunc, output_fn, error_file=None, binary=False):
        ...
        for key in gold.keys():
            max_len = 5
            if len(gold[key]) > max_len:
                gold[key] = gold[key][:max_len]
        ...
        if len(f1s) > 0:
            ...
            if len(temp_rec_scores) == 1 and len(temp_prec_scores) == 1:
                temp_rec_scores.append(temp_rec_scores[0])
                temp_prec_scores.append(temp_prec_scores[0])
...
if __name__ == '__main__':
    ...
    auc_, optimal_f1_point = b.compare(predicted=predicted.oie,
                                       matchingFunc=matchingFunc,
                                       output_fn=out_filename,
                                       error_file=args["--error-file"],
                                       binary=args["--binary"])

    print("AUC: {}\t Optimal (precision, recall, F1): {}".format(auc_, optimal_f1_point))
