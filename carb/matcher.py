...
class Matcher:
    ...
    def linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        predicted_words = ex.pred.split()
        gold_words = ref.pred.split()
        # predicted_words = Matcher.removeStopwords(ex.pred.split())
        # gold_words = Matcher.removeStopwords(ref.pred.split())
        precision[1] += len(predicted_words)
        recall[1] += len(gold_words)
        ...
        for i in range(len(ref.args)):
            gold_words = ref.args[i].split()
            # gold_words = Matcher.removeStopwords(ref.args[i].split())
            if len(gold_words) > 0:
                gold_words_arg += gold_words
                recall[1] += len(gold_words)
            if len(ex.args) <= i:
                continue
            predicted_words = ex.args[i].split()
            # predicted_words = Matcher.removeStopwords(ex.args[i].split())
            if len(predicted_words) > 0:
                predicted_words_arg += predicted_words
                precision[1] += len(predicted_words)
        matching_words = 0
        for w in gold_words_arg:
            if w in predicted_words_arg:
                matching_words += 1
                predicted_words_arg.remove(w)
        precision[0] += matching_words
        # Currently this slightly penalises systems when the reference
        # reformulates the sentence words, because the reformulation doesn't
        # match the predicted word. It's a one-wrong-word penalty to precision,
    ...
    @staticmethod
    def binary_linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):  # matchingFunc(goldEx, predictedEx
        if len(ref.args) >= 2:
            r = copy(ref)
            r.args = [ref.args[0], ' '.join(ref.args[1:])]
        else:
    ...
    with open('../carb/stopwords.txt', 'r', encoding='utf-8') as file:
        lines = file.readlines()
        stop_words = []
        for line in lines:
            stop_words.append(line.strip())
    stopwords = stopwords.words('english') + list(string.punctuation)
    # stopwords = stopwords.words('english') + list(string.punctuation) + stop_words
